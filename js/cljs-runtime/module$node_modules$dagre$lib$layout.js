...
r=g.node(_.last(v.borderRight));v.width=Math.abs(r.x-l.x);v.height=Math.abs(b.y-t.y);v.x=l.x+v.width/2;v.y=t.y+v.height/2}});_.forEach(g.nodes(),function(v){"border"===g.node(v).dummy&&g.removeNode(v)})}function removeSelfEdges(g){_.forEach(g.edges(),function(e){if(e.v===e.w){var node=g.node(e.v);node.selfEdges||(node.selfEdges=[]);node.selfEdges.push({e:e,label:g.edge(e)});g.removeEdge(e)}})}function insertSelfEdges(g){var layers=util.buildLayerMatrix(g);_.forEach(layers,function(layer){var orderShift=
...
x;selfNode=selfNode.height/2;g.setEdge(node.e,node.label);g.removeNode(v);node.label.points=[{x:x+2*dx/3,y:y-selfNode},{x:x+5*dx/6,y:y-selfNode},{x:x+dx,y:y},{x:x+5*dx/6,y:y+selfNode},{x:x+2*dx/3,y:y+selfNode}];node.label.x=node.x;node.label.y=node.y}})}function canonicalize(attrs){var newAttrs={};_.forEach(attrs,function(v,k){newAttrs[k.toLowerCase()]=v});return newAttrs}var _=require("module$node_modules$dagre$lib$lodash"),acyclic=require("module$node_modules$dagre$lib$acyclic"),normalize=require("module$node_modules$dagre$lib$normalize"),
