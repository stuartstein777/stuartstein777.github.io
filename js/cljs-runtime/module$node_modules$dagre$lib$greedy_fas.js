...
function buildState(g,weightFn){var fasGraph=new Graph,maxIn=0,maxOut=0;_.forEach(g.nodes(),function(v){fasGraph.setNode(v,{v:v,"in":0,out:0})});_.forEach(g.edges(),function(e){var prevWeight=fasGraph.edge(e.v,e.w)||0,weight=weightFn(e);fasGraph.setEdge(e.v,e.w,prevWeight+weight);maxOut=Math.max(maxOut,fasGraph.node(e.v).out+=weight);maxIn=Math.max(maxIn,fasGraph.node(e.w)["in"]+=weight)});var buckets=_.range(maxOut+maxIn+3).map(function(){return new List}),zeroIdx=maxIn+1;_.forEach(fasGraph.nodes(),
function(v){assignBucket(buckets,zeroIdx,fasGraph.node(v))});return{graph:fasGraph,buckets:buckets,zeroIdx:zeroIdx}}function assignBucket(buckets,zeroIdx,entry){entry.out?entry["in"]?buckets[entry.out-entry["in"]+zeroIdx].enqueue(entry):buckets[buckets.length-1].enqueue(entry):buckets[0].enqueue(entry)}var _=require("module$node_modules$dagre$lib$lodash"),Graph=require("module$node_modules$dagre$lib$graphlib").Graph,List=require("module$node_modules$dagre$lib$data$list");module.exports=function(g,
weightFn){if(1>=g.nodeCount())return[];weightFn=buildState(g,weightFn||DEFAULT_WEIGHT_FN);weightFn=doGreedyFAS(weightFn.graph,weightFn.buckets,weightFn.zeroIdx);return _.flatten(_.map(weightFn,function(e){return g.outEdges(e.v,e.w)}),!0)};var DEFAULT_WEIGHT_FN=_.constant(1)}
